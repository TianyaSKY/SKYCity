...
class CompanyEmploymentService:
    ...
    def handle_shift_completed(self, session: Session, action: ScheduledAction) -> None:
        ...
        if company.money >= shift.wage_due:
            company.money -= shift.wage_due
            agent.money += shift.wage_due
            shift.wage_paid = shift.wage_due
            shift.payroll_status = "paid"
            session.add(CompanyTransaction(
                world_id=action.world_id, company_id=company.company_id, type="wage_payment",
                amount=-shift.wage_due, balance_after=company.money, related_agent_id=agent.agent_id,
                reference_type="shift", reference_id=shift.shift_id, reason="正式班次工资",
                world_time=world.world_time, trace_id=trace_id,
            ))
            session.add(Transaction(
                world_id=action.world_id, agent_id=agent.agent_id, type="work_wage",
                amount=shift.wage_due, balance_after=agent.money, item_id=None, quantity=None,
                reason=f"{company.name}正式班次工资", world_time=world.world_time, trace_id=trace_id,
            ))
            payroll_event = "wage_paid"
        else:
            shift.payroll_status = "unpaid"
            contract.unpaid_wage += shift.wage_due
            company.unpaid_wage_total += shift.wage_due
            payroll_event = "wage_unpaid"
