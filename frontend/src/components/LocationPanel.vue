    ...
    <div class="location-panel">
        ...
        <template v-if="store.selectedLocation">
            ...
            <dl>
                <div class="row">
                    <dt>类型</dt>
                    <dd>{{ typeLabel(store.selectedLocation.location_type) }}</dd>
                </div>
                <div class="row">
                    <dt>容量</dt>
                    <dd>{{ store.selectedLocation.capacity }}</dd>
                </div>
                <div class="row">
                    <dt>营业</dt>
                    <dd>{{ openHours }}</dd>
                ...
                <div class="row">
                    <dt>状态</dt>
                    <dd>
          <span :class="isOpenNow ? 'open' : 'closed'" class="open-state">
            {{ isOpenNow ? '营业中' : '已关门' }}
          </span>
                    </dd>
                </div>
                <div class="row">
                    <dt>坐标</dt>
                    <dd>({{ store.selectedLocation.col }}, {{ store.selectedLocation.row }})</dd>
                </div>
            </dl>
                ...
                <template v-if="store.locationDetail.products.length">
                    ...
                    <ul class="sec-list">
                        <li v-for="p in store.locationDetail.products" :key="p.item_id" class="product-row">
                            ...
                            <span :class="{ low: p.stock <= 0 }" class="p-stock">×{{ p.stock }}</span>
                        </li>
                    </ul>
                </template>
