...
<template>
  <aside class="company-panel" aria-label="企业总览">
    <header class="cp-head" @click="collapsed = !collapsed">
      <span class="cp-title">企业总览</span>
      <button class="cp-toggle" :title="collapsed ? '展开' : '折叠'">{{ collapsed ? '▸' : '▾' }}</button>
    </header>
    <template v-if="!collapsed">
      ...
      <div v-else class="cp-list">
        ...
        >
          <div class="cp-c-head">
            ...
            <span class="cp-c-status" :class="statusClass(c.status)">{{ statusLabel(c.status) }}</span>
          </div>
          <div class="cp-c-meta">{{ companyTypeLabel(c.company_type) }} · {{ locationName(c.location_id) }}</div>
      ...
      <template v-if="selectedCompany">
        ...
        <div v-if="activeTab === 'overview'" class="cp-detail">
          ...
          <div class="ov-row"><span>欠薪总额</span><b class="warn">{{ selectedCompany.unpaid_wage_total }}</b></div>
          <div class="ov-row"><span>状态</span><b>{{ statusLabel(selectedCompany.status) }}</b></div>
          <div class="ov-row"><span>经理</span><b>{{ selectedCompany.manager_agent_id ? (store.agentById(selectedCompany.manager_agent_id)?.name ?? selectedCompany.manager_agent_id) : '—' }}</b></div>
        ...
        <div v-if="activeTab === 'employees'" class="cp-detail">
          ...
          <table v-else class="cp-table">
            <thead>
              <tr><th>姓名</th><th>岗位</th><th class="num">出勤</th><th class="num">欠薪</th></tr>
            </thead>
            <tbody>
              <tr v-for="e in selectedDetail.employees" :key="e.employment_id">
                ...
                <td class="num" :class="{ warn: e.unpaid_wage > 0 }">{{ e.unpaid_wage }}</td>
              </tr>
            </tbody>
          </table>
        </div>
          ...
          <div v-if="selectedDetail && selectedDetail.positions.length > 0" class="hiring-block">
            ...
            <div v-for="p in selectedDetail.positions" :key="p.position_id" class="pos-item">
              <div class="pos-head">
                ...
                <span class="pos-vac" :class="{ full: p.vacancies === 0 }">
                  {{ p.vacancies }}/{{ p.capacity }} 空缺
                </span>
              </div>
              <div class="pos-meta">每班 {{ p.wage_per_shift }} 金币 · {{ timeOf(p.shift_start_minute) }}–{{ timeOf(p.shift_end_minute) }}</div>
              <p class="pos-desc">{{ p.description }}</p>
            </div>
          </div>
          <div class="hiring-block">
            <div class="hiring-title">公开招聘</div>
            <p v-if="store.jobOpenings.filter((o) => o.company_id === selectedCompany?.company_id).length === 0" class="cp-empty">
              暂无公开招聘
            </p>
            <div
            ...
            >
              ...
              <div class="pos-meta">每班 {{ o.wage_per_shift }} 金币 · {{ timeOf(o.shift_start_minute) }}–{{ timeOf(o.shift_end_minute) }}</div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'ledger'" class="cp-detail">
          ...
          <div v-else class="tx-list">
            <div v-for="t in selectedDetail.transactions" :key="t.transaction_id" class="tx-item">
              ...
              <span class="tx-amount" :class="t.amount >= 0 ? 'pos' : 'neg'">
