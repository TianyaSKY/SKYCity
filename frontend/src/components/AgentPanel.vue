...
<template>
    <aside v-if="store.selectedAgentId" class="agent-panel">
        ...
        <div v-if="activeTab === 'overview'" class="agent-body">
            <div v-if="store.agentDetail" class="ov-card">
                <div class="ov-identity-head">
                    ...
                    <span class="ov-meta">{{
                            store.agentDetail.identity.age
            ...
            <div v-if="liveAgent" class="ov-card">
                ...
                <div class="need-row">
                    ...
                    <span class="need-bar"><i :style="{ width: barWidth(liveAgent.satiety) }"
                                              class="need-fill satiety"/></span>
                    <span class="need-value">{{ liveAgent.satiety }}</span>
                </div>
                <div class="need-row">
                    <span class="need-label">精力</span>
                    <span class="need-bar"><i :style="{ width: barWidth(liveAgent.energy) }" class="need-fill energy"/></span>
                    <span class="need-value">{{ liveAgent.energy }}</span>
                </div>
                <div class="need-row">
                    <span class="need-label">金钱</span>
                    <span class="need-bar"><i :style="{ width: barWidth(liveAgent.money) }"
                                              class="need-fill money"/></span>
                    <span class="need-value">{{ liveAgent.money }}</span>
                </div>
                <div class="need-row">
                    <span class="need-label">心情</span>
                    <span class="need-bar"><i :style="{ width: barWidth(liveAgent.mood ?? 100) }"
                                              class="need-fill mood"/></span>
                    <span class="need-value">{{ liveAgent.mood ?? 100 }}</span>
                </div>
                <div class="need-row">
                    <span class="need-label">孤单</span>
                    <span class="need-bar"><i :style="{ width: barWidth(liveAgent.loneliness ?? 0) }"
                                              class="need-fill loneliness"/></span>
            ...
            <div class="ov-card">
                ...
                <div v-for="d in store.recentDecisions.slice(0, 10)" :key="d.run_id" class="dec-item">
                    ...
                    <span :class="decisionSuccess(d) ? 'ok' : 'fail'" class="dec-ok">{{
                            decisionSuccess(d) ? '✓' : '✗'
                        }}</span>
        ...
        <div v-if="activeTab === 'relationships'" class="agent-body">
            ...
            <i :class="`fill-${axis}`" :style="{ width: barWidth(axisValue(rel, axis)) }" class="rel-axis-fill"/>
          </span>
        ...
        <div v-if="activeTab === 'god'" class="agent-body">
            <div class="ov-card">
                ...
                <div class="god-btns">
                    <button :disabled="godPending" class="god-btn" @click="grantMoney">发放 50 金币</button>
                    <button :disabled="godPending" class="god-btn" @click="deductMoney">扣除 50 金币</button>
                    <button :disabled="godPending" class="god-btn" @click="spawnBread">生成面包×1</button>
                </div>
                <div class="god-row">
                    <select v-model="teleportTarget" :disabled="godPending" class="god-select">
                        <option v-for="loc in teleportLocations()" :key="loc.location_id" :value="loc.location_id">
                            {{ loc.name }}
                        </option>
                    </select>
                    <button :disabled="godPending || !defaultTeleportTarget()" class="god-btn" @click="teleportAgent">
                        传送
                    </button>
                </div>
            </div>
            ...
            <div class="ov-card">
                ...
                <div class="god-row">
                    ...
                    />
                    <button :disabled="godPending || !publicEventText.trim()" class="god-btn" @click="sendPublicEvent">
                        发送
                    </button>
                </div>
                <div class="god-row">
                    <select v-model="stockItem" :disabled="godPending" class="god-select">
                        <option v-for="it in STOCK_ITEMS" :key="it.value" :value="it.value">{{ it.label }}</option>
                    </select>
                    <input v-model.number="stockQty" :disabled="godPending" class="god-qty" min="1" type="number"/>
                    <button :disabled="godPending" class="god-btn" @click="setStoreStock">设置</button>
                </div>
                <div class="god-row">
                    <select v-model="stockTarget" :disabled="godPending" class="god-select">
            ...
            <p v-if="godResult" :class="godResult.success ? 'ok' : 'fail'" class="god-result">
