  ...
  <div class="career-card">
    <template v-if="employment">
      ...
      <div class="career-next">
        下一班次：<b>{{ nextShift ? `${timeOf(nextShift.scheduled_start)} – ${timeOf(nextShift.scheduled_end)}` : '暂无' }}</b>
      </div>
      <div v-if="shifts.length > 0" class="career-shifts">
        ...
        <div v-for="s in shifts.slice(0, 5)" :key="s.shift_id" class="career-shift">
          ...
          <span class="cs-status" :class="`st-${s.status}`">{{ shiftStatusLabel(s.status) }}</span>
