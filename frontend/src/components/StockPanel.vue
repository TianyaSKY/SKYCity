...
<template>
  <aside class="stock-panel" aria-label="小镇股市">
    <header class="sp-head" @click="collapsed = !collapsed">
      <span class="sp-title">小镇股市</span>
      <button class="sp-toggle" :title="collapsed ? '展开' : '折叠'">{{ collapsed ? '▸' : '▾' }}</button>
    </header>
      ...
      <table v-else class="sp-table">
        ...
        <tbody>
          <tr v-for="row in rows" :key="row.stock_id">
            ...
            <td class="num" :style="{ color: row.delta > 0 ? '#81c784' : row.delta < 0 ? '#ff8a65' : 'inherit' }">
