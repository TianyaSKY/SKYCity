...
<template>
  <aside class="task-board" aria-label="任务一览">
    <header class="tb-head">
      <span class="tb-title">任务一览</span>
      <span class="tb-count" :class="{ busy: busyCount > 0 }">{{ busyCount }}/{{ store.agents.length }} 忙碌</span>
    </header>
    ...
    <ul v-else class="tb-list">
      <li
        ...
        @click="select(agent)"
      >
        <span class="tb-dot" :style="{ background: store.agentColors[agent.agent_id] }" />
        <span class="tb-name">{{ agent.name }}</span>
        <span class="tb-task" :class="taskClass(agent)">
