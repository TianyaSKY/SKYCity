...
- 移动耗时对智能体可见：观察的【可见地点】为每个地点标注从当前位置出发的路程耗时，
  供决策权衡；移动完成后该耗时写入智能体记忆（T6-3）。
- 天气不影响其他行动耗时。
...
| 金钱   | 初始 50；工资/交易改变；每日 00:00 扣 `UPKEEP_PER_DAY=120`，余额不足自动负债（余额可为负） |
...
- R20.4 每日生活开销：00:00 每个智能体全额扣除 `UPKEEP_PER_DAY=120` 金币（余额不足时余额变为负数，即负债；
